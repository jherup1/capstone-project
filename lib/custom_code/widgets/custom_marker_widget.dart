...
class RoundImageHelper {
  static Uint8List? getRoundImageFromWeb(int iconSize, http.Response response) {
    ...
    return Uint8List.fromList(img.encodePng(imageFromWeb));
  }
}
...
class _CustomMarkerWidgetState extends State<CustomMarkerWidget> {
  ...
  gm.LatLng calculateCenter() {
    ...
    if (listMarkers.isEmpty) {
      return gm.LatLng(
          widget.positions[0].latitude, widget.positions[0].longitude);
    }
          ...
          if (snapshot.hasData) {
            ...
            g = gm.GoogleMap(
              ...
              initialCameraPosition: gm.CameraPosition(
                  target: gm.LatLng(widget.positions[0].latitude,
                      widget.positions[0].longitude),
                  zoom: 12),
